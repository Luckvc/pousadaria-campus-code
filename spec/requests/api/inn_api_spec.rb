...
describe 'Inn API' do
  ...
  context 'GET /api/v1/inns' do
    ...
    it 'with name search' do
      ...
      expect(json_response[0]["name"]).to eq 'Safari'
      expect(json_response[0]["email"]).to eq 'safari@email.com'
      expect(json_response[0].keys).not_to include("user_id")
      expect(json_response[0].keys).not_to include("cnpj")
      expect(json_response[0].keys).not_to include("company_name")
      expect(json_response[0].keys).not_to include("created_at")
      expect(json_response[0].keys).not_to include("updated_at")
    end
    it 'with city search' do
      host = User.create!(name: 'Lucas', email:'test@email.com', password:'password', host: true)
      address = Address.create!(street: 'Rua das ruas', number:'12', neighborhood:'centro',
        city:'São Paulo', state:'SP', cep:'15470-000')
      inn = host.create_inn!(name:'Safari', company_name:'Safari SN', cnpj:'123',
      ...
      get "/api/v1/inns", params: { query: 'Vinhedo'}

      expect(response.status).to eq 200
      expect(response.content_type).to include 'application/json'
      json_response = JSON.parse(response.body)
      expect(json_response.length).to eq 1
      expect(json_response[0]["name"]).to eq 'Pousada Vineard'
      expect(json_response[0]["phone"]).to eq '223345'
      expect(json_response[0]["email"]).to eq 'pousadona@email.com'
      expect(json_response[0].keys).not_to include("user_id")
      expect(json_response[0].keys).not_to include("cnpj")
      expect(json_response[0].keys).not_to include("company_name")
      expect(json_response[0].keys).not_to include("created_at")
  ...
  context 'GET /api/v1/inns/cities' do
    it 'success' do
      ...
      expect(json_response).to include('Vinhedo')
    end
    it 'empty if no results' do
      get "/api/v1/inns/cities"
